...
const OrderPage = () => {
  ...
  const userCartItems = order.orderItems.filter(item => item.userId === userId);
  ...
  const handleChangeCount = (type, idProduct, limited) => {
    // const item = order.orderItems.find(item => item.product === idProduct && item.userId === userId);
    // if (!item) return;
    if(type === 'increase') {
      if(!limited) {
        dispatch(increaseAmount({idProduct, userId}))
      }
    }else {
      if(!limited) {
        dispatch(decreaseAmount({idProduct, userId}))
      }
    }
  }
